...
- **ticketsatwork** — TicketsAtWork (operated by Entertainment Benefits Group) corporate-perks portal via Patchright. Hotels, theme park tickets, attractions, and live events. Same EBG backend as Working Advantage, Plum Benefits, and Beneplace, so this skill effectively covers all four with one login. Hotel rates often beat direct booking, Booking.com, Chase Travel, and Amex Travel by 10-30% on the same property and dates. Tradeoffs: no portal credits, no card-program elite benefits, no points earning. Theme park tickets show savings claims; exact prices are on the detail page. Live events covers concerts, theater, and sports in destination cities. Rental cars supported but experimental (TaW's car backend often fails to resolve airport codes). Docker: `ghcr.io/borski/ticketsatwork`.
